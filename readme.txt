=== Google AdSense Plugin ===
Contributors: bestwebsoft
Donate link: https://www.2checkout.com/checkout/purchase?sid=1430388&quantity=1&product_id=13
Tags: google, adsense, ads, bestwebsoft, google adsense, google plugin, adsense plugin, ads plugin, gogle
Requires at least: 2.9
Tested up to: 3.3
Stable tag: 1.4

This plugin allows implementing Google AdSense to your website.

== Description ==

Google AdSense Plugin allows to set parameters of ads displaying, such as its format (text advertising, image, text with an image or as links), size, color of the elements at the ad block, rounded corners and also position of the ad block at the website.

<a href="http://wordpress.org/extend/plugins/adsense-plugin/faq/" target="_blank">FAQ</a>
<a href="http://bestwebsoft.com/plugin/google-adsense-plugin/" target="_blank">Support</a>

= Features =

* Display: Ability to adjust appearance of the ads, choose a color scheme, layout, number of ad units per page.
* Actions: Ability to automatically insert google ads to the website.

= Translate =

* Dutch (nl_NL) (thanks Paul Göttgens)
* Russian (ru_RU)

If you create your own language pack or update the existing one, you can send <a href="http://codex.wordpress.org/Translating_WordPress" target="_blank">the text of PO and MO files</a> for <a href="http://bestwebsoft.com/" target="_blank">BWS</a> and we'll add it to the plugin. You can download the latest version of the program for work with PO and MO files  <a href="http://www.poedit.net/download.php" target="_blank">Poedit</a>.

== Installation ==

1. Upload `adsense-plugin` folder to the `/wp-content/plugins/` directory.
2. Activate the plugin through the 'Plugins' menu in WordPress.
3. Site wide Settings are located in "BWS Plugin", "AdSense".

== Frequently Asked Questions ==

= How to change Publisher ID =

Go to the Settings page and change value for the 'Publisher ID' field.

= How to change position of the ad block =

At the page of plugin settings open menu item 'Position & Numbers of Ads' and on the list 'Position' choose appropriate for you position.

== Screenshots ==

1. AdSense Settings page.
2. All menu items at the page are unwrapped.

== Changelog ==

= V1.4 - 24.02.2012 =
* Change : Code to include styles and scripts in a plug for the correct SSL verification.

= V1.3 - 31.01.2012 =
* NEW : Added Dutch language file.

= V1.2 - 27.01.2012 =
* Bugfix : Save settings and display ads on frontend. 

= V1.1 - 29.12.2011 =
* Changed : BWS plugins section. 
* Changed : Default plugin's settings.

= V1.0 - 27.12.2011 =
* NEW : Added all words in language file.

= V0.53 - 24.11.2011 =
* NEW : Add settigns for color of the elements at the ad block.

= V0.52 =
* NEW : Add parameters of ads displaying.

= V0.51 =
* Changed : Usability at the settings page of plugin was improved.

== Upgrade Notice ==

= V1.4 =
Changed code to include styles and scripts in a plug for the correct SSL verification.

= V1.3 =
Added Dutch language file.

= V1.2 =
The bug with save settings and display ads on frontend was fixed in this version. Please upgrade plugin immediately. Thank you

= V1.1 - 29.12.2011 =
BWS plugins section was changed. Default plugin's settings were changed. 

= V1.0 - 27.12.2011 =
Added all words in language file.

= V0.53 =
Add settigns for color of the elements at the ad block.

= V0.52 =
Add parameters of ads displaying.

= V0.51 =
Usability at the settings page of plugin was improved.